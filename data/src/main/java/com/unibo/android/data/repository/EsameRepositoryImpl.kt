...
class EsameRepositoryImpl(context: Context) : EsameRepository {
    ...
    override suspend fun addEsame(esame: Esame): Result<Unit> =
    ...
    override suspend fun updateEsame(esame: Esame): Result<Unit> =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun deleteEsame(esame: Esame): Result<Unit> =
        withContext(Dispatchers.IO) {
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val andamento =
                        dto.chartData.data.mapIndexed { index, voto ->
                            PuntoAndamento(
                                data = LocalDate.now(),
                                voto = voto,
    ...
    override val totalXpFlow: Flow<Int> =
        dao.getTotalXp()
            .map { it ?: 0 }
            .flowOn(Dispatchers.IO)
}
