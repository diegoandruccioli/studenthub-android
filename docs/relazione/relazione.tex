...
Il blocco \texttt{init} lancia un primo caricamento all'avvio del ViewModel.
Per garantire la coerenza dei dati anche dopo reinstallazione o aggiornamento
dell'app --- quando Room è vuota e il primo caricamento potrebbe essere stato
silenziosamente interrotto da una race condition durante l'inizializzazione ---
\texttt{LibrettoScreen} chiama esplicitamente \texttt{refreshEsami()} tramite
\texttt{LaunchedEffect(Unit)}:
