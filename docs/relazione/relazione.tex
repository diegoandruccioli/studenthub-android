...
Room DAO: Flow<List<EsameEntity>>
    ...
    --> ViewModel: stateIn(viewModelScope, Eagerly|WhileSubscribed) --> StateFlow<UiState>
...
corrente viene letto da \texttt{RankDataStore}; \texttt{GamificationRepositoryImpl.getLeaderboard()}
...
\paragraph{Soluzione}
Aggiunto \texttt{.onFailure \{ e -> Log.e(TAG, "...", e) \}} a ogni blocco
...
Per il DELETE l'impatto era funzionale: il blocco \texttt{runCatching} in
\texttt{EsameRepositoryImpl.deleteEsame()} catturava l'eccezione, restituiva
\texttt{null} e il controllo \texttt{response?.isSuccessful == true} risultava
\texttt{false}. L'app chiamava \texttt{markPendingDelete} invece di
\texttt{deleteEsame}, lasciando l'entità in Room con \texttt{pendingDelete = true}
anche se il server aveva già eliminato l'esame con successo. L'esame spariva
dalla UI (filtro \texttt{pending\_delete = 0}), ma \texttt{SyncExamsWorker}
doveva eseguire un ciclo di retry in più prima di rimuoverlo definitivamente da Room.
...
\texttt{Response<ResponseBody>}, in analogia con quanto già adottato per
\texttt{SettingsApiService.updateSettings()}. Il corpo viene chiuso
