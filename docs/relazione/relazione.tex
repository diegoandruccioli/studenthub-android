...
se già attiva).
Il rank precedente è letto da \texttt{RankDataStore} prima della
chiamata al server. A questo punto, \texttt{GamificationRepositoryImpl.getLeaderboard()}
effettua il fetch della classifica e aggiorna \texttt{RankDataStore} come effetto collaterale
tramite \texttt{saveMyRank()}. Il nuovo rank viene quindi riletto e confrontato, garantendo
la totale separazione tra il fetch dei dati globali e il dato statistico puntuale:
...
animazioni di ingresso senza causare jank sul Main Thread e senza ricalcoli onerosi
...
1. \textbf{Mappatura univoca (Single Source of Truth):} I dati di dominio (\texttt{OffsetRelativo})
...
3. \textbf{Gestione dello strato visivo:} Il disegno avviene in ordina rigoroso: prima la griglia
di background, poi l'area del gradiente tramite \texttt{Brush.verticalGradient}, la curva
di Bezier creata dinamicamente interpolando i nodi con \texttt{cubicTo}, e infine il layer
in foreground con le annotazioni.

\begin{lstlisting}
// Animazione per l'ingresso fluido gestita in un LaunchedEffect separato
...
\begin{figure}[H]
    ...
    \caption{Schermata Statistiche (\texttt{StatisticheScreen}): scomposizione dei dati
             accademici (alto) e rendering custom su \texttt{Canvas} dell'andamento voti
             e media ponderata (basso), con legenda integrata.}
