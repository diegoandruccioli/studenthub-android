...
\paragraph{Soluzione}
\texttt{EsameEntity} espone due flag di sincronizzazione:
\texttt{pendingSync: Boolean} (default \texttt{true}) per add/update e
\texttt{pendingDelete: Boolean} (default \texttt{false}) per i delete da ritentare.
Quando la rete è disponibile, l'operazione viene inviata immediatamente tramite
Retrofit; in caso di successo \texttt{markSynced()} azzera \texttt{pendingSync}
e salva il \texttt{remoteId}. Se un DELETE fallisce per assenza di rete,
\texttt{deleteEsame} chiama \texttt{markPendingDelete(id)}: l'esame è nascosto
dalla UI (\texttt{getAllEsami} filtra \texttt{pending\_delete = 0}) ma rimane
in Room per il retry. Analogamente, i record con \texttt{pendingSync = true}
vengono esclusi dalla query \texttt{getPendingDeleteEsami}, e viceversa.

\texttt{SyncExamsWorker} esegue in ordine: (1) retry dei DELETE pendenti ---
se il server risponde 404 l'esame è già eliminato e viene rimosso localmente;
(2) retry degli add/update pendenti con la stessa logica POST/PUT.
Dopo ogni sync riuscito il worker aggiorna le statistiche di gamification e
gli obiettivi. In caso di errori parziali restituisce \texttt{Result.retry()}
per il backoff esponenziale di WorkManager.

La sincronizzazione del catalogo esami in entrata (\texttt{refreshEsami}) usa
una strategia \emph{last-write-wins} con priorità locale: gli esami modificati
sul server vengono aggiornati in Room solo se la copia locale è già sincronizzata
(\texttt{pendingSync = false}) e non è in attesa di eliminazione
(\texttt{pendingDelete = false}). Se la copia locale è in modifica offline
(\texttt{pendingSync = true}) la versione locale vince.
