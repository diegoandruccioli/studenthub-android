...
when (sessionState) {
    ...
    dati ed evitando duplicati o eliminazioni perse in caso di sync concorrente.

    \paragraph{Soluzione}
    La sincronizzazione è gestita tramite due flag booleani direttamente su \texttt{EsameEntity}:
    \texttt{pendingSync} (default \texttt{true}) indica che l'esame non è ancora stato
    confermato dal server; \texttt{pendingDelete} (default \texttt{false}) marca le
    eliminazioni in attesa di conferma remota, nascondendo l'esame dalla UI senza
    rimuoverlo fisicamente da Room fino al completamento del sync.
...
data class EsameEntity(
    ...
    /** true = delete remoto pendente; nascosto in UI, in Room per retry */
    @ColumnInfo(name = "pending_delete") val pendingDelete: Boolean = false,
)
    \end{lstlisting}

    Il DAO espone due query dedicate:
    \texttt{getUnsyncedEsami()} restituisce gli esami con \texttt{pending\_sync = 1}
    e \texttt{pending\_delete = 0};
    \texttt{getPendingDeleteEsami()} quelli con \texttt{pending\_delete = 1} e
    \texttt{remote\_id} non nullo.
    La query \texttt{getAllEsami()} filtra \texttt{WHERE pending\_delete = 0}, così gli
    esami in attesa di eliminazione non sono mai visibili nella UI già prima della
    conferma del server.

    \texttt{SyncExamsWorker} (\texttt{CoroutineWorker}) viene schedulato da WorkManager
    ogni ora con vincolo \texttt{NetworkType.CONNECTED} e viene eseguito anche ad app
    chiusa quando la connessione torna disponibile. Il worker processa le operazioni
    in due fasi ordinate:
    \begin{enumerate}
        \item \textbf{Delete pendenti}: per ogni esame con \texttt{pendingDelete = true},
        chiama \texttt{DELETE /exams/:remoteId}. Sia in caso di successo sia in caso di
        risposta 404 (esame già rimosso lato server), l'esame viene eliminato definitivamente
        da Room. Il 404 viene trattato come successo per evitare loop infiniti.
        \item \textbf{Add/Update pendenti}: per ogni esame con \texttt{pendingSync = true},
        esegue \texttt{POST /exams} se \texttt{remoteId} è \texttt{null} (esame mai
        sincronizzato) oppure \texttt{PUT /exams/:remoteId} se già noto. In caso di
        successo, \texttt{markSynced(localId, remoteId)} aggiorna il \texttt{remoteId}
        e azzera \texttt{pendingSync}.
    \end{enumerate}
    Se almeno un'operazione riesce (\texttt{syncCount > 0}), il worker aggiorna
    in cascata gamification e obiettivi. Se uno o più sync falliscono, restituisce
    \texttt{Result.retry()}, attivando il backoff esponenziale di WorkManager.

    Per il flusso in entrata (\texttt{refreshEsami}), la policy di risoluzione conflitti
    privilegia la versione locale: se un esame ha \texttt{pendingSync = true}, i dati
    provenienti dal server vengono ignorati e la versione locale non viene sovrascritta,
    preservando le modifiche offline dell'utente.
...
% ── CAPITOLO 4 — PUNTI FORTI E PUNTI DEBOLI ─────────────────
    ...
    \begin{itemize}
        ...
        \texttt{pendingDelete} su \texttt{EsameEntity} tracciano ogni azione CRUD offline
        direttamente in Room; \texttt{SyncExamsWorker} le sincronizza in background al ritorno
        della rete con risoluzione dei conflitti a priorità locale.
...
% ── CAPITOLO 5 — MIGLIORIE E SVILUPPI FUTURI ────────────────
    ...
    \begin{itemize}
        ...
        \texttt{EsameEntity}, interrogate frequentemente nelle query di filtering
        del \texttt{SyncExamsWorker}.
    \end{itemize}
